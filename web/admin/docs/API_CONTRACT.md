...
- `POST /admin/devices`
  - body: `{ deviceType, modelName, location: { name, lat, lng }, institutionId?, deliveredAt?, installLocation?, deliveryMemo? }`
- `GET /admin/devices/:deviceId`
  - response extension: `activeInstitution?`, `activeDelivery?`

## 3) Institutions
- `GET /admin/institution-types`
- `GET /admin/institution-types/:typeCode/fields`
- `GET /admin/institutions?query=&typeCode=&status=&hasActiveDevices=&page=&size=`
- `POST /admin/institutions`
- `GET /admin/institutions/:institutionId`
- `PUT /admin/institutions/:institutionId`
- `GET /admin/institutions/:institutionId/deliveries?status=ACTIVE|ENDED`
- `POST /admin/institutions/:institutionId/deliveries`
- `PATCH /admin/institutions/:institutionId/deliveries/:deliveryId/end`
- `GET /admin/institutions/:institutionId/logs?limit=&actionType=&deviceId=`
- `GET /admin/institution-logs?institutionId=&actionType=&deviceId=&limit=`
- `GET /admin/institutions/unassigned-devices?query=&limit=`
...
- fallback command 생성: `POST /api/commands`
  - 기관 계약기간 위반 시 `403` + `message=INSTITUTION_CONTRACT_DATE_DENIED`

## 5) Store Background Sync
